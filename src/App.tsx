...
const SMALL_ITEMS_TO_SPAWN = 10;
const BIG_ITEMS_TO_SPAWN = 4;
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#2a0f1f] via-[#3a1330] to-[#4a183f] text-white flex items-center justify-center p-4">
      <div className="w-full max-w-3xl rounded-3xl bg-black/35 shadow-2xl backdrop-blur p-6 border border-pink-200/20">
        ...
        <div className="mb-3 flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-between">
          <div className="rounded-2xl bg-black/25 border border-pink-200/15 px-4 py-2 text-sm text-pink-50/90 min-h-[44px] flex items-center">
            {toast}
          </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-pink-200/15 bg-black/55 flex justify-center p-3">
          <canvas ref={canvasRef} width={width} height={height} className="max-w-full h-auto rounded-xl" />

          {status !== "playing" && (
            <div
              className={
                status === "reveal"
                  ? "fixed inset-4 sm:inset-8 z-50 rounded-2xl bg-black/90 flex items-center justify-center p-4 sm:p-6 text-center overflow-hidden border border-pink-200/20 shadow-2xl"
                  : "absolute inset-3 rounded-2xl bg-black/85 flex items-center justify-center p-4 sm:p-6 text-center overflow-hidden"
              }
            >
              <div className="max-w-xl w-full max-h-full flex flex-col items-center">
                {status === "start" && (
                  <>
                    {avatarLoaded && <img src="/puthykol-avatar.png" alt="Puthykol avatar" className="mx-auto mb-4 h-24 w-24 rounded-full border-4 border-yellow-200 object-cover shadow-2xl" />}
                ...
                {status === "reveal" && (
                  <>
                    {avatarLoaded && <img src="/puthykol-avatar.png" alt="Puthykol avatar" className="mx-auto mb-4 h-28 w-28 rounded-full border-4 border-pink-200 object-cover shadow-2xl" />}
                    <div className="text-5xl mb-3">💖</div>
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm text-white/75">
          <button onClick={() => (keyRef.current = "up")} className="rounded-xl bg-white/10 p-3 hover:bg-white/20">Up</button>
          <button onClick={() => (keyRef.current = "left")} className="rounded-xl bg-white/10 p-3 hover:bg-white/20">Left</button>
          <button onClick={() => (keyRef.current = "right")} className="rounded-xl bg-white/10 p-3 hover:bg-white/20">Right</button>
          <button onClick={() => (keyRef.current = "down")} className="rounded-xl bg-white/10 p-3 hover:bg-white/20">Down</button>
        </div>
        ...
        <p className="text-xs text-pink-100/75 mt-4 text-center">Use arrow keys, WASD, or the buttons on mobile.</p>
      </div>
    </div>
  );
}
